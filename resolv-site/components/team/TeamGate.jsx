...
// auth user id) gets in. Anyone else is sent to sign in or shown "no access".
export default function TeamGate({ children }) {
  const router = useRouter();
  const pathname = usePathname();
  const [phase, setPhase] = useState('loading'); // loading | ready | denied | error
  ...
  useEffect(() => {
    ...
    const check = async (session) => {
      ...
      const { data, error } = await sb.from('team_members').select('id,name,email,role,status')
        .eq('auth_user_id', session.user.id).maybeSingle();
      if (!alive) return;
      if (error) { setPhase('error'); return; }
      if (!data || data.status !== 'active') { setPhase('denied'); return; }
